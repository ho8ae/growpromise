...
import RewardAchievementModal from '../../components/common/modal/RewardAchievementModal';
...
export default function RewardsScreen() {
  ...
  // 전체 로딩 상태
  const isLoading =
    isRewardsLoading || isStickersLoading || isStatsLoading || isHistoryLoading;
  ...
  // 보상 달성 요청 - 모달 사용
  const handleAchieveReward = async (
    rewardId: string,
    rewardTitle: string,
  ...
  const confirmAchieveReward = (reward: ChildReward) => {
    if (
      stickerStats &&
      stickerStats.availableStickers >= reward.requiredStickers
    ) {
      ...
      Alert.alert(
        '알림',
        `이 보상을 달성하기 위해서는 ${reward.requiredStickers}개의 스티커가 필요합니다.`,
      );
    }
  };
              ...
              <View className="px-5 pt-4 flex-1 flex flex-col justify-between">
                <View className="flex-1">
                    ...
                    >
                      ...
                      {availableStickers.length > 0 ? (
                        <View className="flex-row flex-wrap">
                          {availableStickers
                            .slice(0, Math.min(availableStickers.length, 9))
                            .map((sticker, index) => (
                              <View
                                key={sticker.id}
                                style={{
                              ...
                              >
                                <View className="aspect-square rounded-lg items-center justify-center bg-white border border-gray-100">
                                  <Image
                        ...
                        <View className="flex-row flex-wrap">
                          {Array(6)
                            ...
                            .map((_, i) => (
                              ...
                              >
                                <View
                                  className="aspect-square border-2 border-dashed rounded-lg items-center justify-center"
                                  style={{
                  ...
                  {rewards.length === 0 ? (
                    ...
                    >
                      <View
                        className="items-center justify-center py-8 px-4 rounded-xl"
                    ...
                    >
                      {rewards.map((reward, index) => {
                        ...
                        const progress = Math.min(
                          ((stickerStats?.availableStickers || 0) /
                            reward.requiredStickers) *
                            100,
                          100,
                        );
                              ...
                              <View className="flex-row mb-2">
                                <View
                                  className={`mr-3 w-12 h-12 rounded-full items-center justify-center ${
                                    canAchieve ? 'bg-green-100' : 'bg-gray-100'
                                  }`}
                                >
                                  <MaterialIcons
                                    name={icon as any}
                  ...
                  {rewards.length === 0 && rewardHistory.length === 0 && (
                    ...
                    >
                      <Text
                        ...
                        style={{ color: Colors.light.textSecondary }}
                      >
                        약속을 완료하여 스티커를 모으면 부모님께서 설정한 보상을
                        받을 수 있어요!
                      </Text>
                    </Animated.View>
                  )}
